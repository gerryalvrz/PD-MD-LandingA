...
export function Banner({
  ...
  useEffect(() => {
    ...
    setOpen(!isClosed)
    document.documentElement.classList.toggle(globalKey, isClosed)
    setReady(true)
  }, [globalKey])
  ...
  return (
    <div
      ...
      {...props}
      className={cn(
        "sticky top-0 z-40 flex flex-row items-center justify-center px-4 text-center text-sm font-medium text-white",
        variant === "normal" && "bg-neutral-900/95",
        variant === "rainbow" && "bg-neutral-950",
        !open && "hidden",
        props.className
      )}
      style={{
        ...
        <button
          ...
          className={cn(
            buttonVariants({
              variant: "ghost",
              className:
                "absolute cursor-pointer end-2 md:end-20 top-1/2 -translate-y-1/2 text-white/60 hover:text-white",
              size: "icon",
            })
          )}
        >
